...
interface MonthData { month: string; revenus: number; depenses: number; }
interface CategoryData { name: string; amount: number; pct: number; colorHex: string; }
...
export default function AnalytiquePage() {
      ...
      .then(data => {
        ...
        const cats = Object.entries(data.depensesByCategory || {})
          .map(([name, amount]) => ({
            ...
            pct: data.depenses > 0 ? Math.round(((amount as number) / data.depenses) * 100) : 0,
  ...
  const months = periode === "3 mois" ? monthlyData.slice(-3)
    : periode === "6 mois" ? monthlyData.slice(-6)
    : monthlyData;
  ...
  const maxVal = Math.max(...months.map(d => Math.max(d.revenus, d.depenses)), 1);
  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="flex items-start justify-between">
        <div>
          <h1 className="text-2xl font-bold text-primary" style={{ fontFamily: "var(--font-heading)" }}>Analytique</h1>
      ...
      <div className="bg-white border border-border rounded-2xl p-6">
        <div className="flex items-center justify-between mb-2">
          <h3 className="font-bold text-primary" style={{ fontFamily: "var(--font-heading)" }}>Revenus vs Dépenses</h3>
          <div className="flex gap-4">
            <div className="flex items-center gap-2"><div className="w-2.5 h-2.5 rounded-full bg-success" /><span className="text-xs text-tertiary">Revenus</span></div>
            <div className="flex items-center gap-2"><div className="w-2.5 h-2.5 rounded-full bg-danger" /><span className="text-xs text-tertiary">Dépenses</span></div>
          </div>
        </div>
        ...
        <div className="h-5 mb-3">
          {hoveredMonth !== null && months[hoveredMonth] && (
            <p className="text-xs text-tertiary">
              ...
              <span className="text-success font-semibold">{months[hoveredMonth].revenus.toLocaleString()} €</span>
              {" · "}
              <span className="text-danger font-semibold">{months[hoveredMonth].depenses.toLocaleString()} €</span>
            </p>
          )}
        </div>
        ...
        {months.length === 0 ? (
          <div className="h-48 flex items-center justify-center">
            <p className="text-sm text-tertiary">Aucune donnée disponible. Ajoutez des transactions.</p>
          </div>
        ) : (
              ...
              <div key={i} className="flex-1 flex flex-col items-center gap-1 cursor-pointer"
                ...
                <div className="w-full flex gap-0.5 items-end" style={{ height: "170px" }}>
                  <div className="flex-1 rounded-t-md transition-all"
                    style={{ height: `${(d.revenus / maxVal) * 100}%`, background: hoveredMonth === i ? "#10B981" : "rgba(16,185,129,0.5)" }} />
                  <div className="flex-1 rounded-t-md transition-all"
                    style={{ height: `${(d.depenses / maxVal) * 100}%`, background: hoveredMonth === i ? "#EF4444" : "rgba(239,68,68,0.4)" }} />
                </div>
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <div className="bg-white border border-border rounded-2xl p-6">
          <h3 className="font-bold text-primary mb-5" style={{ fontFamily: "var(--font-heading)" }}>Dépenses par catégorie</h3>
                ...
                <div key={cat.name}>
                  <div className="flex justify-between items-center mb-1.5">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <span className="text-sm font-bold text-primary">{cat.amount.toFixed(0)} €</span>
                    </div>
                  </div>
                  <div className="w-full bg-neutral rounded-full h-1.5">
                    <div className="h-1.5 rounded-full" style={{ width: `${cat.pct}%`, background: cat.colorHex }} />
                  </div>
                </div>
              ))}
            </div>
        ...
        <div className="bg-white border border-border rounded-2xl p-6">
          <h3 className="font-bold text-primary mb-5" style={{ fontFamily: "var(--font-heading)" }}>Comparaison mensuelle</h3>
          {months.length === 0 ? (
